...
def compute_metrics(df, category, model_name, shot, finetuning):
...
def training(dataset, num_epoch, batch_size):
    k_folds = 5
...
def main(model_name, labels, input_dir, num_epoch, batch_size):
    
    data = pd.read_csv(labels)
    data = data.sample(frac=1).reset_index(drop=True)
  
    trainset = preprocess_input(data, model_name)
    
    training(trainset, num_epoch, batch_size)
